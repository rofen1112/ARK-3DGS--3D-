...
- The dev-only texture-fetch draw path now passes on preview PLY and matches the default attribute-buffer output exactly under same-camera signature comparison. Keep it dev-only while moving color/SH into packed textures or validating source-density limits.
