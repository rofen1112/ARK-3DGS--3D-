...
- Added `arkDiagProjection=aholo-material` to test Aholo `SplattingMaterial` defaults (`preBlur=0`, `blur=0.3`, `focalAdjustment=1`) as a combined profile. This does not improve parity: preview worsens from `0.9728` to `1.3020`, and source worsens from `1.6336` to `1.7203`. Keep ARK's current default projection constants for now.
- Added `docs/ARK_GAUSSIAN_PARAMETER_DEBUG_MEMO.md` as the parameter-tuning guardrail. The incident repair and parameter-first debugging phase is closed; future work should use the memo before changing renderer constants.
- Added `docs/ARK_GAUSSIAN_PACKED_DATA_AUDIT.md` to lock the next architecture target: packed covariance textures, order texture access, and packed SH/runtime-channel data paths. Added the first CPU-only `ArkGaussianPackedData` audit module, `validate:packed-data`, and `validate:packed-data:source`; this still does not change default renderer output. Full source PLY packed covariance audit passes with `1,854,627` decoded splats and max covariance delta `6.956697071160362e-7`. The default renderer now publishes data access debug state and QA checks it remains the attribute-buffer path.
- Added dev-only `arkDiagData=texture-audit` and `qa:first-party-data-texture`. The preview PLY now uploads center/covariance/order textures and reads sample texels back from WebGL with covariance max delta `9.151638451498911e-7`; default drawing remains `attribute-buffer`.
- Added dev-only `arkDiagData=texture-fetch`, `qa:first-party-texture-fetch`, and `qa:first-party-texture-fetch-compare`. The preview texture-fetch path renders with `dataPacking="texture-fetch-hybrid"` and is signature-identical to the default `ark-gaussian` path: mean absolute RGB delta `0`, RMS `0`, max RGB delta `0`, similarity `1`.
- Extended the dev-only packed texture path to include color and SH1. `texture-audit` now uploads and readbacks center/covariance/order/color/SH1 textures with `colorMaxAbsDelta=0` and `sh1MaxAbsDelta=0`; `texture-fetch` now reports `colorStorage="color-texture"` and `shStorage="sh1-texture"` while the default renderer remains `color-attribute` and `sh1-attributes`.
- Added `ARK_DEV_SERVER_URL` support to the main first-party QA scripts used in this round, so renderer QA can run against a temporary port when `5173` is occupied by another local app.
- Added `qa:first-party-texture-capacity` as the full-source texture capacity gate. Current source PLY result: WebGL2 `MAX_TEXTURE_SIZE=8192`, the 8-texture source layout fits at `1362 x 1362`, estimated texture memory is `226.446MiB`, and diagnostic load peak is estimated at `1174.63MiB`. This allows a guarded diagnostic smoke only; it does not promote texture-fetch to default.
...
- Treat the packed-data audit as the active implementation guide. The packed covariance round trip is now validated on preview and full source PLY, so the next renderer change should add diagnostic order/texture access without changing the default path.
- GPU texture upload/readback now passes on preview PLY. The next renderer step should be a separate dev-only texture-fetch draw path, followed by same-camera A/B against the current attribute-buffer draw path.
- The dev-only texture-fetch draw path now passes on preview PLY and matches the default attribute-buffer output exactly under same-camera signature comparison. Center, covariance, order, color, and SH1 now all have texture-backed diagnostic coverage. Source-density capacity is now green but medium-pressure, so the next texture step must be a guarded full-source diagnostic smoke with strict memory and timing reporting, not a default path change.
- Treat packed covariance/order texture access and projection data layout as the next focused target. The current isolation pass does not justify further sorting-order, alpha-blend, or Aholo-material-constant tuning.
